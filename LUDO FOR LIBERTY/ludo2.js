    ...
    if (!(path.classList.contains('star-place') || path.classList.contains("tokenStart"))) {
        ...
        tokensInPath.forEach(t => {
            ...
            if (tokenName != otherTokenName) {
                if (confirm("Do you want to kill the token? If yes, answer the question")) {
                    var answer = prompt("which article describes freedom of speech and expression?");
                    if (answer == "14") {
                        killed = true;
                        ...
                        // converting to array to traverse easily 
                        ...
                        alert("Wrong answer! You cannot kill the token.");
                    }
                }
            }
        });
...
function update() {
    ...
    if (extraChance() || killed || homeChance) {
        ...
        // incrementing and resetting the playersMove to loop between 4 players 
        if (playersMove === 4) {
            playersMove = 1;
        } else {
            playersMove++;
    ...
    // to make grid to append more than one token at a place 
    cubePath.forEach(path => {
        let images = path.querySelectorAll('img');
        if (images.length > 4) {
            path.classList.add("makeGrid2");
    ...
    // game loop to recursively call another player 
}
